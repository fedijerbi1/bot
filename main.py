...
def count_rows():
    """How many rows exist? Used to auto-name 'day N'."""
...
def create_sleep_row(sleep_iso):
    ...
    page = notion.pages.create(
        ...
        properties={
            "day": {
                "title": [{"text": {"content": f"day {n}"}}]
            },
            "date": {
                "date": {"start": date.today().isoformat()}
            },
            "sleep Time": {
                "date": {"start": sleep_iso}
            },
        }
    )
    return page["id"]
...
def find_open_row():
    """Find the most recent row that has sleep Time but no wakeUp."""
    ds_id = get_data_source_id()
    result = notion.data_sources.query(data_source_id=ds_id)

    # Sort newest first by created_time
    rows = sorted(
        result["results"],
...
def handle_sleep():
    # Don't create two sleep rows in a row
...
def main():
    ...
    send("🤖 Bot online (v2 — new row per sleep). Send `/help`.")
